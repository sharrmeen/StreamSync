...
const Dashboard: React.FC = () => {
  ...
  const [currentStream, setCurrentStream] = useState<{ id: string; url: string } | null>(null);
  const [loading, setLoading] = useState(true);
  const [currentTime, setCurrentTime] = useState<number>(0); // Start at T=0
  ...
  useEffect(() => {
    ...
    const initializeLiveStream = async () => {
      try {
        // Get the live stream URL with userId and currentTime
        const liveStream = await getLiveStream(userId, currentTime);
        setCurrentStream(liveStream);
        toast({
          title: "Live Stream Ready",
          description: "The live stream is now available. Click to watch in fullscreen.",
    ...
    initializeLiveStream();
  }, [userId, navigate, currentTime]);

  // Update the time as the video plays
  const handleTimeUpdate = (time: number) => {
    if (Math.abs(time - currentTime) >= 2) { // Update every 2 seconds
      setCurrentTime(time);
    }
  };
